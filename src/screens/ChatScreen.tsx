    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card/95 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-semibold text-foreground">EcoBot</h2>
            <p className="text-[10px] text-muted-foreground">Carbon footprint assistant</p>
          </div>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
            ...
            <div>
              <h3 className="text-lg font-semibold text-foreground">Ask me about food carbon footprints</h3>
              <p className="text-sm text-muted-foreground mt-1">
                I know about agricultural practices, transport methods, and CO₂ emissions for food products.
              </p>
            </div>
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="text-xs px-3 py-1.5 rounded-full border border-border bg-card text-foreground hover:bg-secondary transition-colors"
                >
                  {s}
                </button>
              ))}
      ...
      <div className="px-4 py-3 border-t border-border bg-card/95 backdrop-blur-md">
        ...
        >
          <input
            ...
            className="flex-1 h-10 px-4 rounded-full bg-secondary text-sm text-foreground placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-ring"
            disabled={isLoading}
          />
          <Button
            type="submit"
